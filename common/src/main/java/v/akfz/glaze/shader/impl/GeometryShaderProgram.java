...
public class GeometryShaderProgram implements IShaderProgram {
    ...
    private void init() {
        ...
        try {
            ...
            int geometryId = this.createShader(this.geometryLocation, GL_GEOMETRY_SHADER);
            glAttachShader(this.programId, geometryId);
            compiledShaders.add(geometryId);

            int fragmentId = this.createShader(this.fragmentLocation, GL_FRAGMENT_SHADER);
    ...
    private String loadShaderSource(ResourceLocation location) {
        try {
            ...
            if (resourceOptional.isEmpty()) {
                throw new IOException("Shader assets file not found: " + location);
            }
    ...
    public int getUniformLocation(String name) {
        ...
        return this.uniformLocationCache.computeIfAbsent(name, key ->
                glGetUniformLocation(this.programId, key)
        );
    }
    ...
    public void checkActive() {
        if (!this.isInUse) {
            throw new IllegalStateException("Geometry shader program '" + this.getProgramName() + "' is not active!");
        }
    }
