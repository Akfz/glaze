package v.akfz.glaze.pprmodule;
...
public final class PostProcessRenderer implements RenderModule {
    ...
    public void addShader(ResourceLocation fragmentShaderPath) {
        if (!pendingShaders.contains(fragmentShaderPath)) {
            pendingShaders.add(fragmentShaderPath);
    ...
    public void removeShader(ShaderProgram sh) {
        shaders.remove(sh);
    }
    ...
    private void checkInit() {
        if (!initialized && !pendingShaders.isEmpty()) {
            ...
            for (ResourceLocation fragment : pendingShaders) {
                try {
                    shaders.add(new ShaderProgram(vertex, fragment));
                } catch (Exception e) {
    ...
    public void render(Object... args) {
        if (args.length == 0 || !(args[0] instanceof RenderTarget mainBuffer)) return;

        checkInit();

        if (!isEnabled() || shaders.isEmpty()) return;
    ...
    public boolean isEnabled() {
        return enabled && RenderModuleManager.INSTANCE.haveModule(this);
    }
