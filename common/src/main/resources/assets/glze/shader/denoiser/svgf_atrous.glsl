...
void main() {
    ...
    float meanLuma = 0.0;
    float meanLumaSqr = 0.0;
    float sampleCount = 0.0;

    for (int x = -1; x <= 1; x++) {
        for (int y = -1; y <= 1; y++) {
            float luma = getLuminance(texture(uLightTex, vTexCoord + vec2(x, y) * texelSize).rgb);
            meanLuma += luma;
            meanLumaSqr += luma * luma;
            sampleCount += 1.0;
        }
    }
    meanLuma /= sampleCount;
    meanLumaSqr /= sampleCount;
    float variance = max(0.0, meanLumaSqr - meanLuma * meanLuma);
